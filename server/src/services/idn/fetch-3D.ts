...
interface FetchBet3dPlayerParams {
  phpsessid: string
  base_URL: string
  pasaran: any
  website: string
...
const fetch_bet_3D_player = async (params: FetchBet3dPlayerParams) => {
  const {
    base_URL,
    kode_pasar,
    phpsessid: PHPSESSID,
  ...
  while (berhasil) {
    randomChalk(
      `Fetching data bet 3D di pasaran ${pasaran} di ${website} page ke ${page}`
    )
    ...
    const response = await axios<string>(
      `${base_URL}/admin_invoice_frame.php?tombol=3D&start=${start}&by=&end=${end}&s_user=&s_nomor=&s_periode=${periode}&pos2d=B&dist=&bet=&tanggalnya=&usernya=&tebakan=`,
